...
export default function RoomId({ setViewDetails, data2 }) {
  ...
  useEffect(() => {
    const fetchRoomData = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_BACKEND_API}/get-room-password`, {
  ...
  const handleRoomIdAndPass = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${import.meta.env.VITE_BACKEND_API}/create-room-password`, {
